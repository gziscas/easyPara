...
import com.easycms.cms.dao.assist.CmsOracleDataBackDao;
...
public class CmsOracleDataBackDaoImpl extends JdbcDaoSupport implements
	...
	private boolean isDateTimeColumn(String tablename,int columnIndex){
		if(StringUtils.isNotBlank(tablename)){
			...
			if(upperTableName.equals("JC_CONFIG")){
				if(columnIndex==21||columnIndex==22||columnIndex==36||columnIndex==37){
					return true;
				}
			}
			if(upperTableName.equals("JC_CONTENT_BUY")){
				if(columnIndex==9){
					return true;
				}
			}
			if(upperTableName.equals("JC_CONTENT_CHARGE")){
				if(columnIndex==7){
					return true;
				}
			}
			if(upperTableName.equals("JC_CONFIG_CONTENT_CHARGE")){
				if(columnIndex==18){
					return true;
				}
			}
			if(upperTableName.equals("JC_USER_ACCOUNT")){
				if(columnIndex==13||columnIndex==14){
					return true;
				}
			}
			if(upperTableName.equals("JC_ACCOUNT_DRAW")){
				if(columnIndex==7){
					return true;
				}
			}
			if(upperTableName.equals("JC_ACCOUNT_PAY")){
				if(columnIndex==7){
					return true;
				}
			}
