...
import com.easycms.cms.dao.assist.CmsSiteAccessStatisticDao;
...
public class CmsSiteAccessStatisticDaoImpl extends
	...
	public List<Object[]> statisticByYear(Integer year,Integer siteId, 
		...
		Finder f = Finder.create("select sum(bean.pv)as pv,sum(bean.ip)as ip,"
				+ "sum(bean.visitors)as visitors,"
				+ "(sum(bean.visitSecondAver * bean.visitors)/sum(bean.visitors)) as second"
				+ ", month(bean.statisticDate)as m from CmsSiteAccessStatistic bean "
				+ " where bean.site.id=:siteId "
		...
		if(groupByMonth){
			f.append(" group by month(bean.statisticDate) "
					+ "order by month(bean.statisticDate) asc");
		}
		if(orderBy!=null){
			if(orderBy==0){
				f.append(" order by bean.pv desc");
