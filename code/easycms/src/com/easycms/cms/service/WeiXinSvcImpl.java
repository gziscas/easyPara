package com.easycms.cms.service;
...
import com.easycms.cms.Constants;
import com.easycms.cms.entity.main.Content;
import com.easycms.cms.web.CmsThreadVariable;
import com.easycms.common.image.ImageUtils;
import com.easycms.common.upload.FileUpload;
import com.easycms.common.util.PropertyUtils;
import com.easycms.common.web.springmvc.RealPathResolver;
import com.easycms.core.entity.CmsConfig;
import com.easycms.core.entity.CmsSite;
import com.easycms.core.entity.Ftp;
import com.easycms.core.manager.CmsConfigMng;
...
public class WeiXinSvcImpl implements WeiXinSvc {
	...
	public String getToken() {
		String tokenGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),TOKEN_KEY);
		String appid="";
		String secret="";
		CmsSite site=CmsThreadVariable.getSite();
		if(site!=null){
			appid=site.getAttr().get(com.easycms.core.Constants.WEIXIN_APPKEY);
			secret=site.getAttr().get(com.easycms.core.Constants.WEIXIN_APPSECRET);
		}
	...
	public Set<String> getUsers(String access_token) {
		String usersGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),USERS_KEY);
	...
	public String  uploadFile(String access_token,String filePath,String type){
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),UPLOAD_KEY);
	...
	public void sendText(String access_token,String content) {
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),SEND_KEY);
        String url = sendGetUrl+"?access_token=" + access_token;
	...
	public void sendContent(String access_token,String title, String description, String url,
			String picurl) {
		String sendUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),SEND_KEY);
	...
	public void createMenu(String menus){
		...
		String createMenuUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),MENU_KEY);
	...
	public void sendTextToAllUser(Content[] beans){
		String token=weixinTokenCache.getToken();
		//上传内容到微信
		String articalUploadUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),UPLOAD_NEWS);
		     ...
		     client = (CloseableHttpClient) wrapClient(client);
		       ...
		       try{
		           ...
		           if(StringUtils.isNotBlank(media_id)){
		        	   String sendAllMessageUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),SEND_ALL_MESSAGE);
	...
	private String  uploadImg(String access_token,String filePath){
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.EASYCMS_CONFIG)),UPLOAD_IMG_URL);
        String url = sendGetUrl+"?access_token=" + access_token;
		String result = null;
		String mediaId="";
		FileUpload fileUpload = new FileUpload();
